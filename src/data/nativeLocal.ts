...
export const nativeLocalModels: Record<NativeModelId, {
  ...
  "LingoFusion Native-9B": {
    badge: "Balanced · Recommended", downloadSize: "Build-specific", parameters: "9B parameters", context: "Build-specific context", bestFor: "Most users on mainstream 16 GB-class computers", modelMatcher: /qwen.*9b/i, buildSpecific: true,
    windows: ["Windows 10 or Windows 11, 64-bit", "16 GB-class system recommended", "Actual memory and storage needs depend on the selected Qwen 9B quantization", "Modern 64-bit Intel or AMD processor", "GPU optional; compatible GPU can improve speed"],
    mac: ["Apple Silicon recommended", "16 GB-class unified memory recommended", "Actual memory and storage needs depend on the selected Qwen 9B quantization", "Integrated Apple Silicon GPU supported"],
  },
  "LingoFusion Native-35B": {
    badge: "Best Local Quality", downloadSize: "~15 GB", parameters: "35B total parameters · ~3B active", context: "128K context", maxOutput: "32K max output", bestFor: "Difficult translations and powerful local computers", modelMatcher: /(?:qwen|native).*35b/i,
